...
**FR-TXN-015 — Date filter for transaction list**
The transaction list shall be filtered by creation date. `GET /api/transactions` accepts an optional `date` query param (YYYY-MM-DD, interpreted in WIB timezone). When provided, only transactions whose `created_at` falls on that date in WIB are returned. When omitted, the API defaults to today WIB. The frontend filter defaults to today WIB on page load. The date filter row is hidden when the Penugasan tab is active (assignments have no date filter).
...
- **Transaction list:**
  - Date filter above status tabs: `<input type="date">` defaulting to today WIB + conditional "Hari Ini" shortcut button (hidden when already on today); calls `GET /api/transactions?date=YYYY-MM-DD` on change (see FR-TXN-015). Date filter row hidden on Penugasan tab.
