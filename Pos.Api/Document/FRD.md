...
> MSMe Water & Gas | Spec Version 5.15 | App Version 1.1.0 | September 22, 2026
...
|---|---|---|---------|
| 5.15 | September 22, 2026 | — | FR-CST-011 baru — **Pergerakan Stok per pelanggan**. Setiap kartu pelanggan pada halaman Pelanggan kini memiliki tombol **"Pergerakan Stok"** yang membuka modal berisi ringkasan pergerakan stok pelanggan tersebut, mengikuti pola bagian "Pergerakan Stok" pada Dashboard (FR-DSH-012) namun ter-scope ke satu pelanggan. Modal memuat date picker **Tanggal** sebagai jangkar (default hari ini WIB, maksimal hari ini) plus pemilih periode **Harian, Mingguan, Bulanan, Tahunan, Kustom** yang memakai aturan resolusi `StockPeriodRange` yang sama (Mingguan = pekan kalender Senin–Minggu, Bulanan = bulan kalender, Tahunan = tahun kalender, Kustom = rentang Dari/Sampai inklusif; rentang yang tidak lengkap, terbalik, atau di masa depan tidak memicu permintaan dan pesannya tampil sebagai keterangan berwarna danger). Metrik per produk adalah **Terjual** (kontainer terisi yang diantar ke pelanggan; untuk produk simple seluruh kuantitas dispatch) dan **Dikembalikan** (kontainer kosong yang diserahkan pelanggan kembali) — berbeda dari "Diterima" pada FR-DSH-012 yang secara struktural selalu nol untuk satu pelanggan pada produk refillable. Kepemilikan pergerakan ditentukan lewat `StockMovement.TransactionId` menuju `Transaction.CustomerId` karena `StockMovement` tidak memiliki kolom pelanggan, sehingga hanya dispatch, kembalian kontainer, dan baris pembalikannya yang dihitung; pergerakan tanpa transaksi (mis. penerimaan dari vendor) dan pinjaman kontainer manual (`adjustment`) tidak termasuk. Pergerakan yang dibatalkan (`is_reversed` atau `is_reversal`) dikecualikan dan produk tanpa aktivitas dihilangkan. Setiap baris produk dapat diketuk untuk membuka rincian per staf (Terjual dan Dikembalikan) secara akordeon di dalam modal yang sama, bukan modal bertingkat. Endpoint baru `GET /api/customers/{id}/stock-summary` menerima `period`, `date`, `start_date`, `end_date` dan mengembalikan `{ customer_id, customer_name, period, start_date, end_date, items[] }` dengan `items[]` berisi `{ product_id, product_name, product_unit, product_category, total_sold, total_returned, staff[] }`, diurutkan berdasarkan nama produk (baris staf: Terjual menurun lalu nama), `400` untuk rentang tidak valid, dan `404` bila pelanggan tidak ditemukan. Tersedia untuk **semua peran** dan **tanpa polling**. Frontend: modal baru `CustomerStockMovementModal` beserta `CustomerStockMovementModal.module.scss`, tipe `CustomerStockProductItem`, `CustomerStockStaffItem`, dan `CustomerStockSummaryResponse` pada `types/index.ts`, metode `customerService.getStockSummary()` beserta mock-nya, kolom mock-only `customerId` pada tipe `StockMovement`, seed `mockDb.stockMovements` baru agar setiap preset terisi, serta kelas SCSS `.stockBtn` pada `CustomersPage.module.scss`. Tanpa migrasi, tanpa perubahan skema, tanpa aturan validasi baru, dan tanpa bump App Version. |
...
- As any staff, I want to see a customer's stock movement for a day, week, month, year or a custom range so I can verify what I delivered and which containers they returned.

### 8.2 Functional Requirements

**FR-CST-001 — Customer list**
Display all **active** customers (see FR-CST-010) with: name, phone, address (truncated). Because soft-deleted customers are never listed, the list renders no active/inactive status badge and no muted styling — only the "Konfidensial" badge appears (owner view, FR-CST-008).
...
**FR-CST-004 — Hapus pelanggan (soft delete)**
The system shall allow a customer to be **deleted** from the customer list via the "Hapus" action on the customer card. Deletion is implemented as a **soft delete**: `DELETE /api/customers/{id}` sets `is_active = false` and returns `204 No Content`. The row is **retained** so that historical transactions, debt history (`GET /api/customers/{id}/debt-history`), container-loan balances, and customer pricing all remain intact.
- Deleted customers no longer appear in the customer list (FR-CST-001 / FR-CST-010) nor in any customer picker: transaction Step 1 (FR-TXN-001), assignment creation (FR-TXN-016), the Kontainer Manual form (FR-CON-006), and the debt payment form (FR-DBT-006).
- There is **no UI to reactivate** a deleted customer — the previous "Aktifkan" action is removed; restoring a deleted customer is a database-level operation.
- If the customer still has `outstanding_debt > 0`, the confirmation dialog surfaces a warning showing the outstanding amount before the delete proceeds; the delete is still permitted (deleting a customer does not clear their debt).
...
- Pencarian berlaku untuk seluruh pelanggan yang ditampilkan — yaitu pelanggan **aktif** saja (FR-CST-010).
...
**FR-CST-010 — Daftar pelanggan hanya menampilkan pelanggan aktif**
`GET /api/customers` shall return **only customers with `is_active = true`** by default. The `active_only` query parameter defaults to `true`; passing `active_only = false` includes soft-deleted customers (retained records). This applies to every role (owner, kasir, kurir) and composes with the confidential filter of FR-CST-008.
- The frontend customer list (`CustomersPage`) therefore renders active customers only, and the `Aktif` / `Tidak Aktif` status badge plus the muted inactive-card styling are removed.
- Every other consumer of the customer list (transaction Step 1 picker, assignment creation, Kontainer Manual form, debt payment form) already operates on active customers only.
- Backend: `CustomerService.GetAllAsync(bool activeOnly = true, string? userRole = null)`; `CustomersController.GetAll([FromQuery] bool activeOnly = true)`.

**FR-CST-011 — Pergerakan Stok per Pelanggan (modal)**
Every customer card gains a **"Pergerakan Stok"** action that opens a read-only modal titled `"Pergerakan Stok — {customerName}"` showing that customer's movement summary for a period, following the Dashboard "Pergerakan Stok" section (FR-DSH-012) but scoped to a single customer.

- **Access:** all roles (the Customers page is all-roles); the endpoint is available to any authenticated staff member.
- **Anchor date picker (inside the modal):** a **Tanggal** input (`<input type="date">`, default = today WIB, `max` = today) anchors the Harian / Mingguan / Bulanan / Tahunan presets. The Customers page itself gains **no** date filter, and the modal's dates never affect the customer list.
- **Period control (inside the modal):** pill tabs **Harian, Mingguan, Bulanan, Tahunan, Kustom** exposed through `aria-pressed`, resolved with the same rules and messages as FR-DSH-012. Selecting **Kustom** pre-fills both inputs from the range currently in effect; the **Dari**/**Sampai** inputs are capped at today. The resolved range is always rendered as a caption under the tabs (`18 Sep 2026`, `14 – 20 Sep 2026`, `September 2026`, `2026`); an incomplete, inverted or future-dated custom range issues **no request** and replaces the caption with its validation message.
- **Ownership of movements:** `StockMovement` has no customer column, so a movement belongs to the customer through `StockMovement.TransactionId` → `Transaction.CustomerId`. Only movements whose transaction belongs to this customer are counted. Movements with no transaction (e.g. vendor receives) and standalone container loans (`movement_type = 'adjustment'`, linked via `ContainerLoan`) are **not** part of this summary — a deliberate difference from the container-loan ledger of FR-CON.
- **Metrics per product:**
  - **Terjual:** `dispatch` movements — refillable products count `container_status = 'filled'` only; simple products count every dispatch quantity.
  - **Dikembalikan:** inbound **empty**-container movements (`movement_type = 'receive'` and `container_status = 'empty'`), i.e. the empties the customer handed back. This replaces FR-DSH-012's "Diterima", which is structurally `0` for a single customer on refillable products.
  - Cancelled movements (`is_reversed = true` or `is_reversal = true`) are excluded, and products with both totals `0` are omitted.
- **Ordering:** products by name ascending; staff rows by `Terjual` descending, then staff name.
- **Per-staff breakdown (accordion):** tapping a product row expands it **in place** (no nested modal) into a `Staf` / `Terjual` / `Dikembalikan` table grouped by the movement's creator, using `—` for a zero value and `"Tidak ada rincian staf."` when there is nothing to show. Only one product row is expanded at a time, and switching the period collapses it.
- **Refresh:** the modal loads when it opens and whenever the period, anchor date or custom bounds change; it does **not** poll.
- **Empty states:** `"Tidak ada pergerakan stok pada tanggal ini."` for Harian and `"Tidak ada pergerakan stok pada periode ini."` for every other period; failure row `"Gagal memuat pergerakan stok."`
- **Backend:** `GET /api/customers/{id}/stock-summary` with `period` (`day` | `week` | `month` | `year` | `custom`), `date` (anchor; defaults to today WIB) and `start_date` plus `end_date` (both required for `custom`; an omitted `period` with both bounds supplied is treated as `custom`). Response: `{ customer_id, customer_name, period, start_date, end_date, items[] }` where `items` is `[{ product_id, product_name, product_unit, product_category, total_sold, total_returned, staff[] }]` and `staff` is `[{ staff_id, staff_name, sold, returned }]`. Invalid input returns `400` with the shared `StockPeriodRange` message; an unknown customer returns `404`. Implemented by `CustomerService.GetStockSummaryAsync`, reusing `StockPeriodRange.TryResolve`.
- No new table, no migration and no `VAL-CST-` rule — the period messages are the shared FR-DSH-012 ones.
...
- **List page:** Search input (`"Cari nama atau nomor HP..."`) di atas daftar — memfilter berdasarkan nama (case-insensitive) atau nomor telepon (lihat FR-CST-009); hanya pelanggan **aktif** yang dimuat (FR-CST-010), sehingga tidak ada badge status dan tidak ada kartu yang diredupkan; Table — Nama, Telepon, Alamat, Aksi (Edit, Pergerakan Stok, Harga Khusus, Hapus)
- **Pergerakan Stok modal (FR-CST-011, all roles):** tombol **"Pergerakan Stok"** pada setiap kartu pelanggan membuka modal `"Pergerakan Stok — {nama}"` (ukuran `lg`) berisi date picker **Tanggal** (default hari ini WIB), pill **Harian / Mingguan / Bulanan / Tahunan / Kustom**, keterangan rentang terpilih (atau pesan validasi berwarna danger selama rentang kustom belum valid), dan daftar produk. Setiap baris menampilkan nama + satuan (kiri) serta chip merah **"Terjual N"** dan chip hijau **"Dikembalikan N"** (kanan; sebuah chip hanya dirender bila nilainya > 0), dengan chevron yang berotasi 90° saat dibuka. Baris dapat diketuk — atau diaktifkan dengan Enter/Space — untuk membuka rincian per staf secara akordeon di dalam modal yang sama. Modal ditutup lewat tombol **Tutup**, tombol ✕, klik overlay, atau Esc.
- **Pricing page:** Sub-page — satu baris per produk aktif dengan input harga khusus opsional
- **Hapus:** Dialog — `"Hapus [nama]? Pelanggan ini akan dihapus dari daftar dan formulir transaksi."` Bila pelanggan masih memiliki hutang (`outstanding_debt > 0`), dialog menambahkan peringatan: `"⚠ Pelanggan ini masih memiliki hutang Rp X. Menghapus pelanggan tidak menghapus hutangnya."` Penghapusan tetap diizinkan.
...
- **Notifikasi Toast:** Berhasil buat → `"Pelanggan berhasil dibuat."` · Berhasil ubah → `"Pelanggan berhasil diperbarui."` · Hapus → `"Pelanggan berhasil dihapus."` · Simpan harga khusus → `"Harga khusus berhasil disimpan."` · Gagal → `"Terjadi kesalahan. Silakan coba lagi."`
...
2. **Per-item product rows** — each row contains: Product selector, Jumlah Kosong Diserahkan, Jumlah Terisi Diterima, **Harga Beli Satuan (Rp)**, Biaya Pembelian (Rp). Rows can be added with "+ Tambah Produk" and removed (remove button is disabled when only one row remains).
3. **Auto-computed item purchase cost** — the per-item **Biaya Pembelian (Rp)** field is filled automatically as `harga beli satuan × Jumlah Terisi Diterima`. It is recalculated whenever Harga Beli Satuan or Jumlah Terisi Diterima changes. The field keeps its manual-override affordance: once the user types a value into it, auto-calculation pauses for that row; clearing the field re-enables auto-calculation (and immediately re-derives the value from the current harga satuan × jumlah terisi). Harga Beli Satuan itself is optional and UI-only — it is **not** sent to the API.
4. **Shared catatan** (optional) at the bottom, applying to all items
5. **Total Biaya Pembelian** — a read-only computed sum row showing the aggregate purchase cost across all items (`Σ` Biaya Pembelian per item). Because each item total is auto-filled, this row populates automatically as quantities and unit prices are entered.
6. A single **"Simpan Pertukaran"** button that submits all items at once via `POST /api/stock/vendor-exchange/bulk`

The server creates two `StockMovements` records per item atomically (same as single-product `POST /api/stock/vendor-exchange`). The request payload is unchanged: each item still sends `purchase_cost` as its per-item total (whether auto-computed or manually overridden).
...
| VAL-STK-008 | `purchase_cost` | Required for vendor exchange (`POST /api/stock/vendor-exchange` and `/bulk`); must be a positive decimal. On the Tukar Agent form this is the per-item total, auto-derived as `harga beli satuan × Jumlah Terisi Diterima` (manual override allowed) | `"Biaya pembelian wajib diisi untuk pertukaran vendor."` / `"Biaya pembelian harus berupa angka positif."` |
...
| VAL-STK-011 | `unit_purchase_cost` (vendor exchange, UI only) | Optional, integer ≥ 0; captured on the Tukar Agent form to derive the per-item `purchase_cost` (`harga beli satuan × Jumlah Terisi Diterima`). Not sent to the API and not persisted | `"Harga beli satuan harus berupa angka positif."` |
...
- **Vendor exchange form (FR-STK-013):** Shared location picker at top → one or more item rows (each: product, Jumlah Kosong Diserahkan, Jumlah Terisi Diterima, **Harga Beli Satuan (Rp)**, Biaya Pembelian (Rp)) → "+ Tambah Produk" button → shared note → Total Biaya Pembelian computed row → "Simpan Pertukaran" button. Entering **Harga Beli Satuan** and/or **Jumlah Terisi Diterima** auto-fills that row's **Biaya Pembelian** (`harga beli satuan × jumlah terisi`), which stays editable as a manual override — typing a value pauses the auto-calc for that row, clearing the field resumes it. The **Total Biaya Pembelian** row keeps its `Σ` formula over the item totals, so it populates automatically. Calls `POST /api/stock/vendor-exchange/bulk`; the payload still sends `purchase_cost` per item (no API change).
...
- As any staff, I want to see which customers still hold our containers so I can follow up.
- As any staff, I want to see which customers' containers we still hold so I know what to send back on the next delivery.
- As an Owner, I want to see how much each staff member collected per payment method so I can reconcile cash, transfers and QRIS.
- As any staff, I want to view the store's stock movement report per day, week, month, year or a custom range — without disturbing the rest of the dashboard.
...
- **The Pendapatan card is clickable** — activating it (click, or Enter/Space since the card is exposed as `role="button"` with `tabIndex=0`) opens the **"Rincian Metode Pembayaran"** modal (FR-DSH-013), which breaks the revenue down per payment method and — for the owner — per staff member within each method (FR-DSH-015).
...
**FR-DSH-012 — Stock Movement Summary ("Pergerakan Stok", period filter)**
The dashboard shall include a **"Pergerakan Stok"** section positioned above "Stok Gudang". It shows per-product **sold** and **received** totals across the whole store for a **period chosen inside the section itself**. This section is visible to all roles (store-wide, not user-scoped).

**Period filter (scoped to this section only):** the section carries its own period control — a row of pill tabs **Harian**, **Mingguan**, **Bulanan**, **Tahunan** and **Kustom** — defaulting to **Harian**. Changing it affects **only this section**; every other dashboard section keeps using the shared date picker (FR-DSH-006). The shared date picker remains the **anchor** for the presets:
- **Harian** — the selected date.
- **Mingguan** — the calendar week **Senin–Minggu** containing the selected date.
- **Bulanan** — the calendar month containing the selected date (1st → last day).
- **Tahunan** — the calendar year containing the selected date (1 Jan → 31 Dec).
- **Kustom** — the two dates typed in the **Dari** and **Sampai** inputs (inclusive).

Changing the shared date picker re-anchors the selected preset. Selecting **Kustom** pre-fills both inputs with the range currently in effect; both inputs are capped at today (WIB). The resolved range is always rendered as a caption under the tabs (`18 Sep 2026`, `14 – 20 Sep 2026`, `September 2026`, `2026`). An incomplete, inverted or future-dated custom range issues **no request** and replaces the caption with its validation message. Switching the period closes an open detail modal. All ranges are inclusive and resolved in WIB (UTC+7).

**Definitions (applied to the whole resolved range):**
...
**Detail modal:** Clicking a product row opens a modal titled **"Pergerakan Stok — {productName}"** showing the resolved period as a caption and a per-staff sold/received summary table totalled over that period. The table groups the period's movements for that product by `created_by_name`, computing each staff member's sold and received totals using the same definitions above. Rows are sorted by sold descending. The data comes from `GET /api/stock/movements` with `start_date`, `end_date` and `product_id` — the range supersedes `date`, and the product filter keeps a yearly payload small.

**Backend:** new endpoint `GET /api/dashboard/stock-summary` with query params `period` (`day` | `week` | `month` | `year` | `custom`), `date` (anchor; defaults to today WIB) and `start_date` plus `end_date` (both required when `period = 'custom'`; an omitted `period` with both bounds supplied is treated as `custom`). Response: `{ period, start_date, end_date, items[] }` where `items` is `[{ product_id, product_name, product_unit, product_category, total_sold, total_received }]` sorted alphabetically by product name and empty when the range has no activity. Invalid input returns `400` with a message. `daily_stock_summary` is **no longer part of `GET /api/dashboard`**; the section polls the new endpoint independently, so changing the period never refetches the other sections. Per-customer variant: **FR-CST-011** (Customers page) reuses the same period resolver (`StockPeriodRange`) and the same pill/caption UI, but scopes the numbers to one customer and swaps "Diterima" for "Dikembalikan" (empties handed back by that customer).

**FR-DSH-013 — Rincian Metode Pembayaran (modal)**
The **"Pendapatan"** stat card on the Dashboard is clickable (FR-DSH-002) and opens a modal titled **"Rincian Metode Pembayaran"**. The card is exposed as `role="button"` with `tabIndex=0` and a pointer cursor, so it can also be activated with Enter or Space.

The modal displays exactly **three cards**, one per payment method, in the fixed order **Tunai** (`cash`) → **Transfer** (`transfer`) → **QRIS** (`qris`). Each card shows the method icon, the localized label, the aggregated amount (`formatCurrency`) and the transaction count rendered as `"{N} transaksi"`. Methods with no transactions in scope are **still rendered** with `amount = 0` and `count = 0` — the card set is fixed and never filtered.

Only transactions with `status = 'completed'` on the **selected dashboard date** contribute; the amount is the sum of their `paid_amount`. **Role scoping** follows FR-DSH-011: the Owner sees the store-wide breakdown, while Kasir and Kurir see the breakdown of their own transactions only. All roles may open and view the modal.

Dismissal: the "Tutup" button, the ✕ button, clicking the overlay, or pressing Esc. The modal renders from the `payment_method_breakdown` array already returned by `GET /api/dashboard` — **no new endpoint and no extra fetch**.

**FR-DSH-015 — Rincian Pendapatan per Staf per Metode Pembayaran**
Each method card of the "Rincian Metode Pembayaran" modal (FR-DSH-013) shall additionally show a **per-staff income breakdown** for that payment method.

- The card renders one row per staff member who created at least one completed transaction with that payment method on the selected date. Each row shows the **staff name** (left) and the **amount + transaction count** (right, `"{formatCurrency(amount)} · {N} trx"`), with a thin **proportion bar** underneath whose width is `staff.amount ÷ card.amount`.
- Rows are sorted by `amount` **descending**, then by staff name ascending.
- The rows are **read-only**: no navigation, no action button, no tap target.
- When the method has no transactions in scope, the card shows the muted empty state `"Belum ada transaksi"` instead of staff rows.
- **Owner only.** The backend returns `staff` as an **empty array for kasir and kurir** (mirroring the FR-DSH-010 convention for `staff_revenue`), and the frontend additionally gates rendering on the caller's role, so kasir/kurir never see the per-staff list.
- **Backend:** each `payment_method_breakdown[]` item is extended with a `staff` array of `{ staff_id, staff_name, amount, count }`. The owner's list is computed by grouping completed transactions on the selected date by payment method + staff. **No new endpoint and no migration.** The four/six summary cards, the FR-DSH-010 "Pendapatan per Staf" pie chart and the date filter are all unaffected — this is additional detail inside the existing modal only.

**FR-DSH-014 — Ringkasan Pinjaman Kontainer ("Kontainer Pelanggan")**
The dashboard shall include a **"Kontainer Pelanggan"** section positioned immediately below "Hutang Pelanggan" (FR-DSH-009) and above "Pergerakan Stok" (FR-DSH-012). It presents the container balance information of the Stock page Kontainer tab (FR-CON-005) as a **read-only accordion**, store-wide for all roles. It is **not date-filtered** — it always shows the current live container balance (same behavior as FR-DSH-009 and FR-DSH-004).

**Aggregation:** net = `SUM(ContainerLoans.quantity)` per customer + product. Loans with `is_reversed = true` are excluded; pairs whose net is `0` are omitted. Only **active** customers (`is_active = true`) are listed — container balances of soft-deleted customers are not shown on the dashboard, consistent with FR-CST-010.

**Sections:** the list is split into two labelled sections, mirroring FR-CON-005:
- **Net > 0 — "Pelanggan memegang kontainer kami":** the customer still holds our unreturned containers (amber).
- **Net < 0 — "Kontainer pelanggan ada di kami":** we hold the customer's excess empty containers and owe them filled containers on the next delivery (blue). A section-level hint reads: `"Kirim kembali sebagai galon terisi di pengiriman berikutnya"`.

**Ordering:** within each section, customers are sorted by their section total (`Σ|net|`) descending, then by customer name; the products inside a customer row are sorted by `|net|` descending, then by product name.

**Backend:** `GET /api/dashboard` response includes a `container_loans` array: `[{ customer_id, customer_name, product_id, product_name, product_unit, net_quantity }]`, returned store-wide for every role. No new endpoint.

### 13.3 UI Behavior

- **Pergerakan Stok section (FR-DSH-012):** Positioned above "Stok Gudang" and the **only** section carrying a period filter. A pill tab row — Harian, Mingguan, Bulanan, Tahunan, Kustom — with the active tab filled in digital violet and exposed through `aria-pressed`; switching it never touches another section. Under the tabs sits the resolved-range caption, or the custom-range validation message in danger red while the range is invalid; the two `Kustom` date inputs ("Dari" and "Sampai", both capped at today WIB) render only for that tab. One row per product with `total_sold > 0 || total_received > 0` for the resolved range, each showing the product name and unit (left) and two chips (right): a red **"Terjual N"** chip when `total_sold > 0` and a green **"Diterima N"** chip when `total_received > 0`. Rows are clickable and open a per-staff detail modal totalled over the same range, repeating the range caption under the modal title. A yearly payload is bounded by the `product_id` filter on the movements endpoint. While a newly selected range loads, the section shows a small spinner; on failure it shows `"Gagal memuat pergerakan stok."`, and the last successful payload renders only while it still matches the selected range. Shown to all roles. Empty states: `"Tidak ada pergerakan stok pada tanggal ini."` for Harian and `"Tidak ada pergerakan stok pada periode ini."` for every other period.
- **Pendapatan stat card (FR-DSH-013):** clickable — `role="button"`, `tabIndex=0`, pointer cursor; Enter/Space also activate it. Opens the **"Rincian Metode Pembayaran"** modal; the card itself keeps showing today's revenue and the revenue % delta.
- **Rincian Metode Pembayaran modal (FR-DSH-013):** one card per payment method — Tunai (green), Transfer (blue), QRIS (orange) — in the fixed order cash → transfer → qris. Each card shows the method icon, the uppercase label, the total amount and `"{N} transaksi"`. Methods with zero transactions still render with `Rp0` / `0 transaksi`. Dismissed via the "Tutup" button, the ✕ button, an overlay click or Esc. Data comes from the existing `payment_method_breakdown` payload — no extra fetch.
- **Per-staff rows inside the modal (FR-DSH-015):** owner only. Inside each method card, below a divider and the `"Per Staf"` label, one row per staff member — name (left), `formatCurrency(amount) · "{N} trx"` (right), and a 4px proportion bar scaled against that method's total; sorted by amount descending. Kasir/kurir see no staff rows. Per-method empty state: `"Belum ada transaksi"`.
...
- **Date filter:** Date picker rendered at the top of the dashboard page; defaults to today. Changing the date immediately re-fetches `GET /api/dashboard?date=YYYY-MM-DD` and updates all sections, and it also re-anchors the period preset of the "Pergerakan Stok" section (FR-DSH-012) without changing which period that section has selected. A `"Hari Ini"` shortcut button resets the filter to today.
...
- **Container loans section (FR-DSH-014):** Positioned below "Hutang Pelanggan" and above "Pergerakan Stok"; always shows the current store-wide container balance; not date-filtered; shown to all roles. Each section renders one **collapsed row per customer** — tapping the row expands or collapses it to reveal one row per product. Customer row: name (left) plus the summary `"{N} produk · {M} unit"` (N = number of product rows in that section, M = `Σ|net|`) and a chevron rotated 90° when open. Product row: product name and unit (left); net value with unit (right, amber when net > 0, blue when net < 0) followed by the note `"belum dikembalikan"` (net > 0) or `"kontainer mereka ada di kami"` (net < 0). Rows are **read-only** — no tap navigation and no action button; recording a return stays on the Stock page Kontainer tab (FR-CON-005/006). The expand/collapse state is tracked per section and survives the 5-second polling refresh. A section with no customers is not rendered. Empty state (no non-zero net at all): `"Tidak ada transaksi kontainer aktif."`
...
| **Customers** — Stock movement summary (FR-CST-011) | ✅ | ✅ | ✅ |
...
| StockMovement | `purchase_cost` | DECIMAL(15,2) | positive; required for vendor exchange; on the Tukar Agent form derived from `harga beli satuan × Jumlah Terisi Diterima` (manual override allowed) | Conditional |
